...
, CASE WHEN p.EstSousTraitance = '1' 
THEN dbo.f_idOptionValeurPlanche(p.IDPlanchePrincipale, 80) 
ELSE dbo.f_idOptionValeurPlanche(p.IDPlanche, 80) END              AS NbCouleursRecto 
, CASE WHEN p.EstSousTraitance = '1' 
THEN dbo.f_idOptionValeurPlanche(p.IDPlanchePrincipale, 81) 
ELSE dbo.f_idOptionValeurPlanche(p.IDPlanche, 81) END              AS NbCouleursVerso 
...
, p.AvecRainage 
, p.AvecPliage*/ 
, dbo.f_bPlancheAvecOptionSaisie(p.IDPlanche,'285,302,303',p.AvecPerforation) AS AvecPerforation 
, dbo.f_bPlancheAvecOptionSaisie(p.IDPlanche,'282,300,301',p.AvecRainage) AS AvecRainage 
, dbo.f_bPlancheAvecOptionSaisie(p.IDPlanche,'284,290,291',p.AvecPliage) AS AvecPliage 
, CASE WHEN p.AvecDecorticageA2 = 1 OR p.AvecDecorticageA3 = 1 OR p.AvecDecorticageA4 = 1 THEN 1 ELSE 0 END AS AvecDecorticage 
, CASE WHEN dbo.f_nIDProduitOptionValeurPlanche(p.IDPlanche, 105, 1) = 519 THEN 1 ELSE 0 END AS AvecDecoupeOutil 
...
, dbo.f_nPlancheNbFeuillesTotal(p.IDPlanche)                         AS NbrFeuilleTotal 
, p.DateAjout 
, a.Nom                                                              AS NomAtelier 
, (SELECT SUM(Cout) 
...
LEFT JOIN TBL_PRODUIT_ACTIVITE_PRODUCTION_TRAD act 
ON act.IDProduitActiviteProduction = p.IDProduitActiviteProduction AND act.IDLangue = 1 
